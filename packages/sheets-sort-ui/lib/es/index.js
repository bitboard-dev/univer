...
import { useDependency as R, IConfirmService as it, useObservable as ct, MenuItemType as f, getMenuHiddenObservable as we, ContextMenuPosition as at, ContextMenuGroup as lt, RibbonStartGroup as dt, IMenuManagerService as ut, IDialogService as ht, ILayoutService as gt, IUIPartsService as Ct, ComponentManager as mt, connectInjector as vt } from "@univerjs/ui";
...
function Wt({ state: e }) {
  const t = R(Ie), r = R(H), n = R(C), [o, s] = P(!1), [i, a] = P(0), l = Ee(null), { range: u, unitId: _, subUnitId: v } = e.location, E = n.getTitles(o), [m, U] = P([
    { type: $.ASC, colIndex: u.startColumn }
  ]), j = T((d, S) => {
    const A = [...m];
    S === null ? A.splice(d, 1) : A[d] = S, U(A);
  }, [m]), h = T(
    ze(() => {
      const d = [...m], S = zt(u, m);
      S !== null && (d.push({ type: $.ASC, colIndex: S }), U(d));
    }, 200),
    [m, u]
  ), z = T((d, S) => {
...
function qt(e) {
  ...
  const m = t.length > 1, U = (j = n.find((h) => h.index === r.colIndex)) == null ? void 0 : j.label;
...
const Ue = "sheet.menu.sheets-sort", Ae = "sheet.menu.sheets-sort-ctx", Z = "AscendingIcon", de = "ExpandAscendingIcon", ue = "DescendingIcon", he = "ExpandDescendingIcon", ge = "CustomSortIcon";
function Xt(e) {
  return {
    id: Ue,
    type: f.SUBITEMS,
