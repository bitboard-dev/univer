...
export class Minifs extends BaseFunction {
    ...
    override calculate(minRange: FunctionVariantType, ...variants: FunctionVariantType[]): BaseValueObject {
        ...
        if (criteriaMaxRowLength === 1 && criteriaMaxColumnLength === 1) {
            return this._scalarMinifs(_minRange as BaseValueObject, _variants);
        }
    ...
    private _scalarMinifs(minRange: BaseValueObject, variants: BaseValueObject[]): BaseValueObject {
        if (!minRange.isArray()) {
            return this._fallbackMinifs(minRange, variants);
        }

        const minArr = minRange as ArrayValueObject;
        ...
        if (canUseRawPath) {
            for (let r = 0; r < rowCount; r++) {
                for (let c = 0; c < colCount; c++) {
                    ...
                    if (allMatch) {
                        const v = minArr.getNumberDirect(r, c);
    ...
    private _fallbackMinifs(minRange: BaseValueObject, variants: BaseValueObject[]): BaseValueObject {
