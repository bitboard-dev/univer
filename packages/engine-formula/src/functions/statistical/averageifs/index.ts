...
export class Averageifs extends BaseFunction {
    ...
    override calculate(averageRange: FunctionVariantType, ...variants: FunctionVariantType[]): BaseValueObject {
        ...
        if (criteriaMaxRowLength === 1 && criteriaMaxColumnLength === 1) {
            return this._scalarAverageifs(_averageRange as BaseValueObject, _variants);
        }
    ...
    private _scalarAverageifs(avgRange: BaseValueObject, variants: BaseValueObject[]): BaseValueObject {
        if (!avgRange.isArray()) {
            return this._fallbackAverageifs(avgRange, variants);
        }

        const avgArr = avgRange as ArrayValueObject;
        ...
        if (canUseRawPath && isScalarFastPathEnabled()) {
            for (let r = 0; r < rowCount; r++) {
                for (let c = 0; c < colCount; c++) {
                    ...
                    if (allMatch) {
                        total += avgArr.getNumberDirect(r, c);
                        count++;
                    }
                }
            }
            ...
            for (let r = 0; r < rowCount; r++) {
                for (let c = 0; c < colCount; c++) {
                    ...
                    if (allMatch) {
                        const cell = avgArr.get(r, c);
                        if (cell && cell.isNumber()) {
                            total += cell.getValue() as number;
                            count++;
                        }
                    }
                }
        ...
        return NumberValueObject.create(total / count);
    }

    private _fallbackAverageifs(avgRange: BaseValueObject, variants: BaseValueObject[]): BaseValueObject {
        const booleanResults = getBooleanResults(variants, 1, 1, true);
        return this._aggregateResults(avgRange, booleanResults);
    }
