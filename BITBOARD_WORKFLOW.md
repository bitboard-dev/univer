...
- `bitboard-v0.10.14` - your custom branch based on v0.10.14 (CURRENT - matches Bitboard production)
- Future: `bitboard-v0.11.x` when upgrading

## Custom Changes

Current customizations in `bitboard-v0.10.14`:

1. **Version suffix:** All packages versioned as `0.10.14-bitboard.1`
...
```json
{
  "dependencies": {
    ...
    "@univerjs/sheets": "0.10.14-bitboard.1",
...
When you want to upgrade to a new upstream version (e.g., v0.11.0):
...
```bash
# Update all package versions to 0.11.0-bitboard.1
...
```bash
# Option A: Cherry-pick your custom commits from v0.10.14
git cherry-pick 947dd7dc6  # Version + toolbar changes commit
...
# Push to fork
git push origin bitboard-v0.11.0
...
```bash
# Update from 0.10.14-bitboard.1 to 0.10.14-bitboard.2
find packages -name "package.json" -type f -exec sed -i '' 's/"version": "0\.10\.14-bitboard\.1"/"version": "0.10.14-bitboard.2"/g' {} \;
sed -i '' 's/"version": "0\.10\.14-bitboard\.1"/"version": "0.10.14-bitboard.2"/g' package.json

git add .
git commit -m "chore: bump to 0.10.14-bitboard.2"
pnpm publish -r --registry http://localhost:4873
```
...
git push origin bitboard-v0.10.14

# Compare with upstream version
git diff v0.10.14..HEAD

# List modified packages
git diff --name-only v0.10.14..HEAD | grep package.json
```
